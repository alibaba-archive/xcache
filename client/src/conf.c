...
conf_create(void)
{
	...
	conf->compiler_check = x_strdup("content");
                 ...
                 void *context)
{
	...
	reformat(&s, "remote_conf = %s", conf->remote_conf);
	printer(s, conf->item_origins[find_conf("remote_conf")->number], context);

	reformat(&s, "remote_only = %s", bool_to_string(conf->remote_only));
	printer(s, conf->item_origins[find_conf("remote_only")->number], context);
	...
	reformat(&s, "read_only_remote = %s",
	         bool_to_string(conf->read_only_remote));
	printer(s, conf->item_origins[find_conf("read_only_remote")->number],
	        context);
