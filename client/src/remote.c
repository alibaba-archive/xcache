...
{
    return x_set(strlen(key), key, len, data);
}


int xcache_set(const char *key,
               const char *obj,
               const char *stderr,
               const char *dia,
               const char *dep,
...
#undef PROCESS_ONE_BUFFER
   return x_set(strlen(key), key, buf_len, buf);
}

static void *xcache_prune(const char *key)
{
    x_del(strlen(key), key);
    return NULL;
}
...
void *xcache_raw_get(const char *key, char **data, size_t *size)
{
    void *value;
    int nvalue = 0;
    value = x_get(strlen(key), key, &nvalue);
    if(NULL == value){
        return NULL;
    }
    *data = value;
...
void *xcache_get(const char *key,
                 char **obj,
                 char **stderr,
                 char **dia,
                 char **dep,
