...
static const char VERSION_TEXT[] =
  MYNAME " version %s\n"
  "\n"
#ifdef ENABLE_REMOTE_CACHE
	"remote_cache support: yes\n"
#else
	"remote_cache support: no\n"
#endif
  "\n"
  "Copyright (C) 2002-2007 Andrew Tridgell\n"
  "Copyright (C) 2009-2018 Joel Rosdahl\n"
...
#ifdef ENABLE_REMOTE_CACHE
static void from_remote_cache(enum fromcache_call_mode mode,
                           bool put_object_in_manifest);
static void to_remote_cache(struct args *args);

#ifdef HAVE_XCACHE
static RemoteInit remote_cache_init       = xcache_init;
static RemoteRawSet remote_cache_raw_set  = xcache_raw_set;
static RemoteSet remote_cache_set         = xcache_set;
static RemoteRawGet remote_cache_raw_get  = xcache_raw_get;
static RemoteGet remote_cache_get         = xcache_get;
static RemoteFree remote_cache_free       = xcache_free;
static RemoteRelease remote_cache_release = xcache_release;
#else
static RemoteInit remote_cache_init       = memccached_init;
static RemoteRawSet remote_cache_raw_set  = memccached_raw_set;
static RemoteSet remote_cache_set         = memccached_set;
static RemoteRawGet remote_cache_raw_get  = memccached_raw_get;
static RemoteGet remote_cache_get         = memccached_get;
static RemoteFree remote_cache_free       = memccached_free;
static RemoteRelease remote_cache_release = memccached_release;
#endif
...
void update_manifest_file(void)
{
	...
	if (manifest_put(manifest_path, cached_obj_hash, included_files)) {
		...
		if (x_stat(manifest_path, &st) == 0) {
			...
			char *data;
			size_t size;
			if (!str_eq(conf->remote_conf, "")
			    && !conf->read_only_remote
			    && read_file(manifest_path, st.st_size, &data, &size)) {
				cc_log("Storing %s in remote database", manifest_key);
				remote_cache_raw_set(manifest_key, data, size);
				free(data);
			}
#endif
		}
...
to_fscache(struct args *args)
{
	...
	if (!str_eq(conf->remote_conf, "")
		...
		cc_log("Storing %s in remote database", cached_key);
...
// Run the real compiler and put the result in cache.
//
// TODO: Too much code duplication between to_fscache and to_remote_cache
static void
to_remote_cache(struct args *args)
{
	if (generating_coverage) {
		cc_log("No remote database support for coverage yet");
		failed();
	}
	if (using_split_dwarf) {
		cc_log("No remote database support for split dwarf yet");
		failed();
	}
	...
	if (remote_cache_set(cached_key, obj_d, stderr_d, dia_d, dep_d,
...
calculate_object_hash(struct args *args, struct mdfour *hash, int direct_mode)
{
	...
	if (direct_mode) {
		...
		if (stat(manifest_path, &st) != 0) {
			...
			size_t size;
			if (!str_eq(conf->remote_conf, "")) {
				cc_log("Getting %s from remote database", manifest_key);
				cache = remote_cache_raw_get(manifest_key, &data, &size);
			}
			if (!cache) {
				return NULL;
			}
...
from_fscache(enum fromcache_call_mode mode, bool put_object_in_manifest)
{
	...
	if (stat(cached_obj, &st) == 0) {
		...
		if (!str_eq(conf->remote_conf, "")
		    && !using_split_dwarf
		    && !generating_coverage) {
			cc_log("Getting %s from remote database", cached_key);
			cache = remote_cache_get(cached_key,
			                       &data_obj, &data_stderr, &data_dia, &data_dep,
			                       &size_obj, &size_stderr, &size_dia, &size_dep);
		}
		if (!cache) {
...
// then this function exits with the correct status code, otherwise it returns.
//
// TODO: Too much code duplication between from_fscache and from_remote_cache.
static void
from_remote_cache(enum fromcache_call_mode mode, bool put_object_in_manifest)
{
	...
	size_t size_dia;
	size_t size_dep;
	void* cache = remote_cache_get(cached_key,
	                             &data_obj, &data_stderr, &data_dia, &data_dep,
	                             &size_obj, &size_stderr, &size_dia, &size_dep);
	if (!cache) {
		return;
...
#ifdef ENABLE_REMOTE_CACHE
	if (!str_eq(conf->remote_conf, "")) {
		if(!remote_cache_init(conf->remote_conf)){
    		if (conf->remote_only) {
    			from_cache = from_remote_cache;
    			to_cache = to_remote_cache;
    		}
        }else{
            cc_log("Disable Remote Cache!\n");
            free(conf->remote_conf);
            conf->remote_conf = x_strdup("");
        }
	}
#endif
...
ccache(int argc, char *argv[])
{
	...
	// Don't hit remote database twice.
	if (conf->remote_only && object_hash_from_manifest
	    && file_hashes_equal(object_hash_from_manifest, object_hash)) {
...
ccache_main_options(int argc, char *argv[])
{
	...
	while ((c = getopt_long(argc, argv, "cChF:M:o:psVz", options, NULL)) != -1) {
		...
		case 'V': // --version
            ...
            fprintf(stdout, "%s\nComplied at: %s %s\n","Version at: 2.1 on 2019/6/12",__DATE__,__TIME__);
			x_exit(0);
