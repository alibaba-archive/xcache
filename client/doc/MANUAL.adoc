...
*remote_conf* (*CCACHE_REMOTE_CONF*)::

    The remote_conf option sets the memcached(3) configuration to use for
    storing and getting cache values, if any. Example configuration:
+
-------------------------------------------------------------------------------
CCACHE_REMOTE_CONF=--SERVER=localhost:11211
-------------------------------------------------------------------------------
+
To use SASL authentication with your memcached, set the variables:
*CCACHE_MEMCACHED_USERNAME* and *CCACHE_MEMCACHED_PASSWORD*.
Note that this also requires using the binary protocol of memcached.
See: <https://blog.couchbase.com/sasl-memcached-now-available/>.
If using Couchbase < 5, use the name of the _bucket_ as the username.
...
*read_only_remote* (*CCACHE_READONLY_MEMCACHED* or *CCACHE_NOREADONLY_MEMCACHED*), see <<_boolean_values,Boolean values>> above)::
