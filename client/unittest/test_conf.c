...
TEST(conf_create)
{
	...
	CHECK_STR_EQ("", conf->remote_conf);
	CHECK(!conf->remote_only);
...
TEST(conf_read_valid_config)
{
	...
	CHECK_STR_EQ("--SERVER=localhost", conf->remote_conf);
	CHECK(conf->remote_only);
...
TEST(conf_print_items)
{
	...
	CHECK_STR_EQ("remote_conf = mc", received_conf_items[n++].descr);
	...
	CHECK_STR_EQ("read_only_remote = false", received_conf_items[n++].descr);
